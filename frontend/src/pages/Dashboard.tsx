...
const ActivityRow: React.FC<{
  ...
  id: string;
  blockNumber?: number;
}> = ({ type, status, time, id, blockNumber }) => {
...
export const Dashboard: React.FC = () => {
  ...
  const aleoBalance = useMemo(
    () => (Number(balances.credits) / 1_000_000).toFixed(3),
    [balances.credits],
  );
  const totalBorrower =
  ...
  const utilizationRate = useMemo(() => {
    ...
    return Math.round(
      (allPublicLoans.filter((l) => l.active).length / allPublicLoans.length) *
        100,
    );
  }, [allPublicLoans]);
  ...
  const liquidationRisk = useMemo(() => {
    ...
    return Math.round(
      (active.filter((l) => currentBlock > l.deadline).length / active.length) *
        100,
    );
        ...
        >
          <Box>
            ...
            <Stack direction="row" alignItems="center" spacing={1.5} mt={0.3}>
              ...
              <Typography
                variant="caption"
                sx={{ color: "#475569", fontSize: "0.72rem" }}
              >
                {lastUpdatedLabel}
              </Typography>
            </Stack>
                ...
                {(["borrower", "lender"] as const).map((t) => (
                  <Button
                    ...
                    sx={{
                      ...
                      bgcolor:
                        tab === t ? "rgba(245,158,11,0.2)" : "transparent",
                      border:
                        tab === t
