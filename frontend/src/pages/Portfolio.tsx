...
import {
  Box, Typography, Paper, Stack, Grid, Chip, Avatar, Button,
  Divider, IconButton, LinearProgress, Tab, Tabs, CircularProgress,
  TextField, Modal, Fade, Backdrop,
} from '@mui/material';
import {
  AccountCircle, OpenInNew, TrendingUp,
  Lock, CurrencyExchange, Shield, Timeline,
  ContentCopy, Visibility, VisibilityOff, Grade,
...
// ─── Balance Card ─────────────────────────────────────────────────────────────
const BalanceCard: React.FC<{ label: string; value: string; unit: string; color: string; icon: React.ReactNode }> = ({ label, value, unit, color, icon }) => (
  <Paper sx={{
    ...
    <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
      <Box>
        <Typography variant="caption" color="text.secondary" sx={{ textTransform: 'uppercase', letterSpacing: '0.08em', fontSize: '0.68rem' }}>
          {label}
        </Typography>
        <Typography variant="h4" fontWeight={800} sx={{ color, fontFamily: 'monospace', mt: 0.5, letterSpacing: '-0.02em' }}>
          {value}
        </Typography>
        <Typography variant="body2" color="text.disabled">{unit}</Typography>
      </Box>
      <Box sx={{ width: 44, height: 44, borderRadius: '12px', background: `${color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', color }}>
        {icon}
      </Box>
    </Stack>
  </Paper>
...
const LoanSummaryRow: React.FC<{ label: string; count: number; color: string }> = ({ label, count, color }) => (
    ...
    <Modal open={open} onClose={onClose} closeAfterTransition
      ...
      <Fade in={open}>
        <Box sx={{
          ...
          width: 360, borderRadius: '20px', outline: 'none',
          ...
          {avatar && (
            <Button size="small" onClick={() => setAvatar(null)}
              sx={{ mb: 2, color: '#ef4444', fontSize: '0.75rem' }}>
              Remove photo
            </Button>
          )}
...
export const Portfolio: React.FC = () => {
  ...
  const {
    ...
    balances, transactionHistory, refreshData, loading,
  } = usePrivLend();
  ...
  const aleoBalance = useMemo(() => (Number(balances.credits) / 1_000_000).toFixed(4), [balances.credits]);
  const totalBorrower = useMemo(() => activeUserLoans.length + expiredUserLoans.length + settledUserLoans.length, [activeUserLoans, expiredUserLoans, settledUserLoans]);
  const totalLender   = useMemo(() => activeLenderLoans.length + expiredLenderLoans.length, [activeLenderLoans, expiredLenderLoans]);
  const totalCollateralLocked = useMemo(() => [...activeUserLoans, ...expiredUserLoans].filter(l => l.collateral_locked).length, [activeUserLoans, expiredUserLoans]);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      } catch { setUsername(`User_${address.slice(5, 11)}`); }
    } else {
  ...
  const saveProfile = (newUsername: string, newAvatar: string | null) => {
    ...
    setAvatar(newAvatar);
    localStorage.setItem(`profile_${address}`, JSON.stringify({ username: newUsername, avatar: newAvatar }));
  ...
  return (
    <Box>
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <Typography variant="h4" fontWeight={800} color="white" letterSpacing="-0.02em" mb={0.5}>Portfolio</Typography>
        <Typography color="text.secondary" mb={4}>Your complete private lending activity on Aleo Testnet</Typography>
      </motion.div>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <Stack spacing={3}>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 }}>
              <Paper sx={{ p: 3, borderRadius: 4, background: 'linear-gradient(145deg,#1e293b,#0f172a)', border: '1px solid rgba(245,158,11,0.2)' }}>
                <Stack direction="row" spacing={2} alignItems="center" mb={2}>
                  ...
                  <Box sx={{ position: 'relative', flexShrink: 0 }}>
                    <Avatar
                      ...
                      sx={{
                        width: 64, height: 64, fontWeight: 800, fontSize: '1.1rem',
                        background: 'linear-gradient(135deg,#F59E0B,#8b5cf6)',
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
              <Paper sx={{ p: 3, borderRadius: 4, background: 'linear-gradient(145deg,#1e293b,#0f172a)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                  <Typography fontWeight={700} color="white">Portfolio Health</Typography>
                  <Typography variant="h5" fontWeight={800} sx={{ color: healthColor, fontFamily: 'monospace' }}>{healthScore}</Typography>
                </Stack>
                <LinearProgress variant="determinate" value={healthScore}
                  sx={{ borderRadius: 4, height: 8, bgcolor: 'rgba(255,255,255,0.06)', '& .MuiLinearProgress-bar': { borderRadius: 4, bgcolor: healthColor } }} />
                <Typography variant="caption" color="text.disabled" mt={1} display="block">
                  {healthScore >= 80 ? '✅ Healthy — no expired loans' : healthScore >= 50 ? '⚠️ Some loans at risk' : '🚨 Liquidation risk detected'}
                </Typography>
              </Paper>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}>
              <Paper sx={{ p: 3, borderRadius: 4, background: 'linear-gradient(145deg,#1e293b,#0f172a)', border: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <Typography variant="caption" color="text.disabled" sx={{ textTransform: 'uppercase', letterSpacing: '0.08em' }}>As Borrower</Typography>
                ...
                <Typography variant="caption" color="text.disabled" sx={{ textTransform: 'uppercase', letterSpacing: '0.08em' }}>As Lender</Typography>
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <Paper sx={{ p: 3, borderRadius: 4, background: 'linear-gradient(145deg,rgba(245,158,11,0.08),rgba(245,158,11,0.02))', border: '1px solid rgba(245,158,11,0.2)' }}>
                ...
                <Stack direction="row" spacing={1} mt={2} flexWrap="wrap" gap={1}>
                  {[{ tier: 'A', color: '#10b981', rate: '0–5%' }, { tier: 'B', color: '#f59e0b', rate: '5–10%' }, { tier: 'C', color: '#ef4444', rate: '10–20%' }].map(t => (
                    <Chip key={t.tier} size="small" label={`Tier ${t.tier} — ${t.rate}`}
        ...
        <Grid size={{ xs: 12, lg: 8 }}>
          <Stack spacing={3}>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 }}>
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <BalanceCard label="ALEO Balance" value={aleoBalance} unit="ALEO (from private records)" color="#f59e0b" icon={<CurrencyExchange />} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <BalanceCard label="USDCx Balance" value={balances.usdcx.toString()} unit="USDCx (stablecoin)" color="#F59E0B" icon={<TrendingUp />} />
                </Grid>
              </Grid>
            </motion.div>
            ...
            {/* Loans + Activity Tabs */}
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
              <Paper sx={{ borderRadius: 4, overflow: 'hidden', background: 'linear-gradient(145deg,#1e293b,#0f172a)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{
                  px: 2, pt: 1,
                  '& .MuiTab-root': { color: 'text.secondary', fontWeight: 600, fontSize: '0.85rem', textTransform: 'none', minWidth: 'auto', px: 2 },
                  ...
                  <Tab label={`Activity (${transactionHistory.length})`} />
                </Tabs>
                    ...
                    <>
                      ...
                      {tab === 2 && (
                        transactionHistory.length === 0 ? (
                          <Box textAlign="center" py={6}>
                            <Timeline sx={{ fontSize: 40, color: 'rgba(255,255,255,0.1)', mb: 2 }} />
                            <Typography color="text.secondary" mb={1}>No transactions found.</Typography>
                            {address && (
                              <Typography
                                component="a"
                                ...
                                variant="caption"
                                sx={{ color: '#F59E0B', textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}
                              >
                          ...
                          <Stack spacing={0.5}>
                            {transactionHistory.map(tx => {
                              const txColor = tx.status === 'Completed' ? '#10b981' : tx.status === 'Pending' ? '#f59e0b' : '#ef4444';
                              return (
                              <Stack
                                key={tx.id}
                                component="a"
                                ...
                                sx={{ py: 1.5, px: 1.5, borderRadius: 2, textDecoration: 'none',
                                  '&:hover': { bgcolor: 'rgba(245,158,11,0.08)', cursor: 'pointer' },
                                  transition: 'background 0.15s',
                                }}>
                                <Stack direction="row" alignItems="center" spacing={1.5}>
                                  <Box sx={{
                                    width: 8, height: 8, borderRadius: '50%', flexShrink: 0,
                                    bgcolor: txColor, boxShadow: `0 0 5px ${txColor}`,
                                  }} />
                                  <Box>
                                    <Typography variant="body2" fontWeight={500} color="white">{tx.type}</Typography>
                                ...
                                <Stack alignItems="flex-end" spacing={0.3}>
                                  <Chip label={tx.status} size="small" sx={{
                                    height: 20, fontSize: '0.65rem', fontWeight: 600,
                                    bgcolor: `${txColor}20`, color: txColor, border: `1px solid ${txColor}40`,
                                  }} />
      ...
      <EditProfileModal
        ...
        username={username} avatar={avatar} onSave={saveProfile}
      />
    </Box>
  );
};
