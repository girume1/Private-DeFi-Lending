...
const PrivLendContext = createContext<PrivLendContextType | undefined>(
  undefined,
);
...
function sumRecordAmounts(records: any[], label?: string): bigint {
  ...
  if (label) {
    ...
    if (unspent.length > 0)
      console.log(
        `[PrivLend] Sample record:`,
        JSON.stringify(unspent[0], null, 2),
      );
  }
...
export const PrivLendProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const activeLenderLoans = useMemo(
    () => lenderLoans.filter((l) => l.active && currentBlock <= l.deadline),
    [lenderLoans, currentBlock],
  );
  const expiredLenderLoans = useMemo(
    () => lenderLoans.filter((l) => l.active && currentBlock > l.deadline),
    [lenderLoans, currentBlock],
  );
  ...
  const loadTransactions = useCallback(async () => {
    try {
      ...
      if (e?.message?.toLowerCase().includes("permission")) {
        console.info(
          "[PrivLend] Transaction history skipped — wallet permission not granted.",
        );
      } else {
  ...
  const fetchRecords = useCallback(
    async (programId: string): Promise<any[]> => {
  ...
  const refreshBalances = useCallback(async () => {
    if (!connected || !address) {
      setBalances({ usdcx: 0n, credits: 0n });
      return;
    }
    ...
    if (nb.credits === 0n) {
      try {
        ...
        if (priv > 0n) {
          console.log(`[PrivLend] ✅ Private credits: ${priv}`);
          nb.credits = priv;
        }
      } catch (e) {
        console.warn("[PrivLend] credits fallback error:", e);
      }
    }
    ...
    if (nb.usdcx === 0n) {
      try {
        ...
        if (priv > 0n) {
          console.log(`[PrivLend] ✅ Private USDCx: ${priv}`);
          nb.usdcx = priv;
        } else
          console.warn(
            "[PrivLend] ⚠️ USDCx records found but amount=0. Check console above for raw record structure.",
          );
      } catch (e) {
        console.warn("[PrivLend] USDCx fallback error:", e);
      }
    }
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        setCurrentBlock(await service.getLatestBlock());
      } catch {}
    };
    poll();
  ...
  useEffect(() => {
    if (!loading) refreshDataRef.current();
  }, [connected, address]);
