...
import {
  Paper, Typography, TextField, Button, Slider, Box, Divider, Stepper, Step, StepLabel,
  CircularProgress, Alert, AlertTitle, MenuItem, Select, InputLabel, FormControl, FormHelperText
} from '@mui/material';
...
export const LoanCreationForm: React.FC<Props> = ({
  ...
  const requestRecordPlaintexts: ((programId: string) => Promise<any>) | undefined =
    walletContext.requestRecordPlaintexts;
  ...
  const { currentBlock, loanCounter, refreshData, balances, service } = usePrivLend();
  ...
  const fetchRecords = async (): Promise<AleoRecord[]> => {
    ...
    if (requestRecords) {
      try {
        ...
        console.log(`[PrivLend] requestRecords(plaintext=true) (${walletName}): ${records.length} records`);
        if (records.length > 0) {
          console.log('[PrivLend] Sample record keys:', Object.keys(records[0]));
          return records;
        }
      } catch (e) {
    ...
    if (requestRecordPlaintexts) {
      try {
        ...
        records = Array.isArray(result) ? result : result?.records ?? [];
        console.log(`[PrivLend] requestRecordPlaintexts (${walletName}): ${records.length} records`);
    ...
    if (requestRecords && records.length === 0) {
      try {
        ...
        console.log(`[PrivLend] requestRecords(no plaintext) (${walletName}): ${records.length} records`);
      } catch (e) {
  ...
  const checkCreditTier = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (records.length === 0) {
        ...
        setDebugInfo(`${records.length} record(s) found but none qualify as CreditTier`);
      }
      return false;
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (BigInt(formData.collateral) > balances.credits) {
      toast.error(`Insufficient microcredits balance. You have ${balances.credits.toString()} microcredits`);
      return;
    }
    ...
    try {
      ...
      console.log('[PrivLend] Full tier record:', JSON.stringify(tierRecord, null, 2));
      ...
      console.log('[PrivLend] Fresh loan counter:', freshCounter, '→ new loan ID:', newLoanId);
      ...
      try {
        ...
        if (ciphertext && decrypt) {
          console.log('[PrivLend] Decrypting CreditTier via wallet decrypt()...');
          creditInput = await decrypt(ciphertext);
          console.log('[PrivLend] Decrypted creditInput:', creditInput);
        } else if ((tierRecord as any).plaintext && typeof (tierRecord as any).plaintext === 'string') {
      ...
      console.log('[PrivLend] open_loan inputs:', {
        newLoanId, blockInt, lender: formData.lender,
        credit: typeof creditInput === 'string' ? creditInput.slice(0, 30) + '...' : '[record object]',
        principalInt, collateralInt, interestInt, durationInt
      });
    ...
    >
      ...
      {!hasEnoughCredits && (
        <Alert severity="error" sx={{ mb: 2 }}>
          ...
          You need {formData.collateral.toLocaleString()} microcredits for collateral.
          Current balance: {balances.credits.toString()} microcredits.
        </Alert>
      )}
      ...
      {isUnsupportedWallet && (
        <Alert severity="error" sx={{ mb: 2 }}>
          ...
          <strong>{wallet?.adapter?.name ?? 'Your wallet'}</strong> does not support
          reading private records required for loan creation.{' '}
          Please switch to <strong>Leo Wallet</strong> or <strong>Shield Wallet</strong>.
        </Alert>
      )}
      ...
      <Grid container spacing={4}>
          ...
          <FormControl fullWidth sx={{ mb: 3 }} disabled={loading}>
            ...
            >
              {INTEREST_PRESETS.map(p => (
                <MenuItem key={p.bps} value={p.bps}>{p.label}</MenuItem>
              ))}
            </Select>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Typography gutterBottom>
            Collateral (microcredits) — min {minCollateral.toLocaleString()} (150%)
          </Typography>
          <Slider
          ...
          <FormControl fullWidth sx={{ mb: 3 }} disabled={loading}>
            ...
            >
              {DURATION_PRESETS.map(p => (
                <MenuItem key={p.blocks} value={p.blocks}>{p.label}</MenuItem>
              ))}
            </Select>
            <FormHelperText>
              Current block: {currentBlock > 0 ? currentBlock.toLocaleString() : '...loading'}
            </FormHelperText>
          </FormControl>
        </Grid>
      </Grid>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Your balance: <strong>{balances.credits.toString()} microcredits</strong>
          </Typography>
        </Box>
