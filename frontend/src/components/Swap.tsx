...
import {
  ...
  Alert, CircularProgress, InputAdornment, Stack, Chip
} from '@mui/material';
...
export const Swap: React.FC<Props> = ({ onSuccess, onClose }) => {
  ...
  const [direction, setDirection] = useState<'credits-to-usdcx' | 'usdcx-to-credits'>('credits-to-usdcx');
  // User enters in ALEO (not microcredits) for credits direction
  const [amountIn, setAmountIn] = useState('');
  const [loading, setLoading] = useState(false);

  // Display balances in human-readable form
  const aleoDisplay = useMemo(() => (Number(balances.credits) / 1_000_000).toFixed(6), [balances.credits]);
  const usdcxDisplay = useMemo(() => balances.usdcx.toString(), [balances.usdcx]);
  ...
  const hasEnough = useMemo(() => {
    ...
    if (direction === 'credits-to-usdcx') return rawAmountIn <= balances.credits;
    return rawAmountIn <= balances.usdcx;
  }, [rawAmountIn, direction, balances]);

  const toggleDirection = () => {
    setDirection(d => d === 'credits-to-usdcx' ? 'usdcx-to-credits' : 'credits-to-usdcx');
    setAmountIn('');
  };
    ...
    new Promise((resolve, reject) => {
      ...
      const interval = setInterval(async () => {
        ...
        try {
          ...
          const s = (typeof status === 'string' ? status : (status as any)?.status)?.toLowerCase() ?? '';
          ...
          if (s.includes('failed') || s.includes('rejected') || s.includes('error')) {
            clearInterval(interval); reject(new Error('Transaction rejected')); return;
          }
          if (attempts >= 20) { clearInterval(interval); reject(new Error('Transaction timed out')); }
        } catch {}
      }, 3000);
    });
  ...
  const handleSwap = async () => {
    ...
    if (rawAmountIn <= 0n) { toast.error('Enter a valid amount'); return; }
    if (!hasEnough) {
    ...
    try {
      const functionName = direction === 'credits-to-usdcx'
      ...
      const inputs = direction === 'credits-to-usdcx'
  ...
  return (
    <Paper sx={{
      ...
      <Stack direction="row" alignItems="center" spacing={2} mb={3}>
        ...
        <Typography variant="h5" fontWeight={700}>Swap Tokens</Typography>
        <Chip label="1:1 Testnet Rate" size="small"
          sx={{ ml: 'auto', bgcolor: 'rgba(16,185,129,0.1)', color: '#10b981', border: '1px solid rgba(16,185,129,0.2)', fontSize: '0.7rem' }} />
      </Stack>

      {/* From */}
      <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.06)', mb: 1 }}>
        <Stack direction="row" justifyContent="space-between" mb={1}>
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            <Typography variant="caption" color="text.disabled">Balance: {fromBal}</Typography>
            <Button size="small" onClick={setMax}
              sx={{ fontSize: '0.65rem', py: 0.2, px: 1, minWidth: 0, height: 20, bgcolor: 'rgba(245,158,11,0.15)', color: '#FCD34D', borderRadius: 1 }}>
              MAX
            </Button>
          </Stack>
        </Stack>
        <Stack direction="row" alignItems="center" spacing={2}>
          <TextField
            ...
            sx={{
              '& .MuiInput-root': { fontSize: '1.8rem', fontWeight: 700, color: !hasEnough && amountIn ? '#ef4444' : 'white' },
              '& .MuiInput-root::before': { display: 'none' },
              '& .MuiInput-root::after': { display: 'none' },
            }}
          />
          <Box sx={{
            px: 2, py: 1, borderRadius: 2, flexShrink: 0,
            bgcolor: direction === 'credits-to-usdcx' ? 'rgba(245,158,11,0.15)' : 'rgba(245,158,11,0.15)',
            border: `1px solid ${direction === 'credits-to-usdcx' ? 'rgba(245,158,11,0.3)' : 'rgba(245,158,11,0.3)'}`,
          }}>
            <Typography fontWeight={700} sx={{ color: direction === 'credits-to-usdcx' ? '#f59e0b' : '#FBBF24' }}>
              {fromLabel}
            </Typography>
          </Box>
        </Stack>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', my: 1 }}>
        <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <Box onClick={toggleDirection} sx={{
            width: 36, height: 36, borderRadius: '50%', cursor: 'pointer',
            bgcolor: '#111827', border: '2px solid #334155',
      ...
      {/* To */}
      <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.06)', mb: 3 }}>
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          <Typography sx={{ fontSize: '1.8rem', fontWeight: 700, color: '#6ee7b7', flex: 1 }}>
            {estimatedOut || '0'}
          </Typography>
          <Box sx={{
            px: 2, py: 1, borderRadius: 2, flexShrink: 0,
            bgcolor: direction === 'usdcx-to-credits' ? 'rgba(245,158,11,0.15)' : 'rgba(245,158,11,0.15)',
            border: `1px solid ${direction === 'usdcx-to-credits' ? 'rgba(245,158,11,0.3)' : 'rgba(245,158,11,0.3)'}`,
          }}>
            <Typography fontWeight={700} sx={{ color: direction === 'usdcx-to-credits' ? '#f59e0b' : '#FBBF24' }}>
              {toLabel}
            </Typography>
          </Box>
        </Stack>
      ...
      <Alert severity="info" sx={{ mb: 3, borderRadius: 2, fontSize: '0.8rem' }}>
        ...
        {direction === 'credits-to-usdcx' && ' (e.g. "1.5" = 1.5 ALEO = 1,500,000 microcredits)'}
      </Alert>
      ...
      <Button
        ...
        sx={{
          ...
          background: isValid ? 'linear-gradient(135deg, #F59E0B, #8b5cf6)' : undefined,
          boxShadow: isValid ? '0 4px 20px rgba(245,158,11,0.3)' : undefined,
        }}
        startIcon={loading ? <CircularProgress size={20} color="inherit" /> : <SwapIcon />}
      >
