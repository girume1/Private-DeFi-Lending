...
function extractLoanId(record: PrivateLoanRecord): string | number | undefined {
  ...
  if (
    ...
    (record.data as any).includes("loan_id")
  ) {
...
function recordMatchesLoanId(
  record: PrivateLoanRecord,
  loanId: number,
): boolean {
...
function extractRawData(record: PrivateLoanRecord): Record<string, any> {
  if (
    (record.data as any)?.value &&
    typeof (record.data as any).value === "object"
  )
  ...
  if (typeof record.data === "string")
    return parsePlaintextStr(record.data as any);
  return {};
}
...
export const LoanCard: React.FC<{
  ...
  const requestRecordPlaintexts:
    ...
    | undefined = walletCtx.requestRecordPlaintexts;
  ...
  const [estimatedInterestBps, setEstimatedInterestBps] =
    useState<bigint>(500n);
  const [estimatedDurationBlocks, setEstimatedDurationBlocks] =
    useState<number>(0);
  ...
  const interestSoFar = useMemo(() => {
    ...
    return (
      (estimatedPrincipal * estimatedInterestBps * elapsed) /
      (10000n * duration)
    );
  }, [
    estimatedPrincipal,
    estimatedInterestBps,
  ...
  const handleRepay = async () => {
    ...
    try {
      ...
      let loanRecord = loanRecords.find((r) =>
        recordMatchesLoanId(r, loan.loan_id),
      );
      if (!loanRecord) {
        const sorted = [...loanRecords].sort(
          (a, b) =>
        ...
        console.log(
          "[PrivLend] Matched by block order, using record at index",
          loan.loan_id - 1,
        );
      }
      ...
      console.log(
        "[PrivLend] principal from record:",
        principal.toString(),
        "| interestBps:",
        interestBps.toString(),
      );
      ...
      if (interestBps === 0n) {
        interestBps = 500n;
        console.warn(
          "[PrivLend] interest_bps unknown, defaulting 500bps (may be wrong)",
        );
      }
      if (durationBlocks === 0) {
      ...
      // Use min(currentBlock, deadline) so Max payment is always valid even if
      // a few blocks pass between estimation and on-chain execution.
      ...
      let interestAmount =
        (principal * interestBps * elapsed) / (10000n * duration);
      ...
      const ciphertext =
        (loanRecord as any).recordCiphertext ??
        (loanRecord as any).ciphertext ??
        "";
      const existingPlaintext =
        typeof (loanRecord as any).plaintext === "string"
      ...
      try {
        if (ciphertext && decrypt) {
          console.log(
            "[PrivLend] Decrypting Loan record via wallet decrypt()...",
          );
          loanInput = await decrypt(ciphertext);
          console.log(
            "[PrivLend] Decrypted loanInput (first 60):",
            String(loanInput).slice(0, 60),
          );
          ...
          throw new Error(
            "No valid Loan record found. Ensure your wallet has access to this program's records.",
          );
        }
      } catch (decryptErr: any) {
        console.warn(
          "[PrivLend] decrypt() failed, trying plaintext fallback:",
          decryptErr,
        );
        loanInput = existingPlaintext || ciphertext;
        if (!loanInput)
          throw new Error(
            "Could not obtain a valid Loan record input for repayment.",
          );
      }
      ...
      if (typeof loanInput === "string" && loanInput.trim().startsWith("{")) {
        ...
        const decDurationBlocks = Number(
          parseAleoNumber(decoded.duration_blocks),
        );
        if (decPrincipal > 0n) {
          ...
          durationBlocks =
            decDurationBlocks > 0 ? decDurationBlocks : durationBlocks;
          // Cap elapsed at duration so Max payment is always valid
          const effectiveBlockDec = Math.min(
            currentBlock,
            startBlock + durationBlocks,
          );
          const elapsedDec = BigInt(
            Math.max(0, effectiveBlockDec - startBlock),
          );
          const durationDec = BigInt(Math.max(1, durationBlocks));
          interestAmount =
            (principal * interestBps * elapsedDec) / (10000n * durationDec);
          ...
          setEstimatedStartBlock(startBlock);
          console.log(
            "[PrivLend] totalDue re-derived from decrypted plaintext (time-based):",
            totalDue.toString(),
          );
        }
      }
      ...
      console.log(
        "[PrivLend] repay_loan — loanInput type:",
        String(loanInput).startsWith("{") ? "plaintext" : "ciphertext",
        "| totalDue:",
        totalDue.toString(),
      );
      ...
      if (principal === 0n) {
        throw new Error(
          "Cannot determine loan principal from record. Check browser console for details.",
        );
      }
      ...
      if (actualPayment > totalDue) {
        throw new Error(
          `Payment ${actualPayment} exceeds total due ${totalDue}`,
        );
      }
      if (actualPayment > balances.usdcx) {
        throw new Error(
          `Insufficient USDCx balance. Need ${actualPayment.toString()}, have ${balances.usdcx.toString()}`,
        );
      }

      // Use the same capped block for the tx input — ensures on-chain total_due matches our calculation
      ...
      if (!repayOk)
        throw new Error(
          "Repayment was rejected on-chain. The payment amount may not match the contract's calculated total due. Try refreshing and repaying again.",
        );
  ...
  const handleLiquidate = async () => {
    if (!connected) return toast.error("Connect wallet first");
    if (!isLender && !isExpired)
      return toast.error("Only the lender can liquidate expired loans");
    ...
    try {
      ...
      const collateralRecords = records
        .filter((r) => {
          if (r.spent) return false;
          const name = (r as any).recordName ?? (r as any).name ?? "";
          const fn = (r as any).functionName ?? "";
      ...
      if (colAmount === 0n) {
        const ciphertext =
          (collateralRecord as any).recordCiphertext ??
          (collateralRecord as any).ciphertext ??
          "";
        const existingPlaintext =
          typeof (collateralRecord as any).plaintext === "string"
        ...
        try {
          ...
          console.warn("[PrivLend] Could not decrypt Collateral record:", e);
        }
      }

      if (colAmount === 0n) {
        throw new Error(
          "Could not determine collateral amount from Collateral record.",
        );
      }
    ...
    <motion.div whileHover={{ y: -5 }}>
      ...
      >
        <CardContent>
          ...
          <Box
            sx={{ mt: 2, p: 1.5, bgcolor: "rgba(0,0,0,0.2)", borderRadius: 2 }}
          >
            <Typography variant="body2" color="text.secondary">
              Collateral:
            </Typography>
            <Typography
              variant="h6"
              color={loan.collateral_locked ? "warning.light" : "success.light"}
            >
                ...
                <>
                  ...
                  {totalDueEstimate > 0n && (
                    <Box sx={{ display: "flex", gap: 0.75, mb: 0.5 }}>
                      {[
                        ...
                        const amt = (totalDueEstimate * pct) / 100n;
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      try {
                        ...
                        if (parsed <= 0n) {
                          ...
                          totalDueEstimate > 0n &&
                          parsed > totalDueEstimate
                        ) {
                          setPaymentError(
                            `Cannot exceed total due (${totalDueEstimate.toString()})`,
                          );
                        } else {
                          setPaymentError("");
                        }
                    ...
                    helperText={
                      ...
                      (totalDueEstimate > 0n &&
                      parsedPayment > 0n &&
                      parsedPayment < totalDueEstimate
