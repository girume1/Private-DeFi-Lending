...
function useAnimationFrame(callback: UseAnimationFrameCallback): void {
  useEffect(() => {
    let frameId: number;
    const loop = () => {
      callback();
...
function useMousePositionRef(containerRef: React.RefObject<HTMLElement>) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (ev: { clientX: any; clientY: any; }) => updatePosition(ev.clientX, ev.clientY);
