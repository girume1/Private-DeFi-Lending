...
export const PROGRAM_ID    = import.meta.env.VITE_PROGRAM_ID    || 'privlend_v5.aleo';
export const NETWORK       = import.meta.env.VITE_NETWORK       || 'testnet';
export const API_ENDPOINT  = import.meta.env.VITE_API_ENDPOINT  || 'https://api.explorer.provable.com/v2';
export const USDCX_PROGRAM = import.meta.env.VITE_USDCX_PROGRAM || 'test_usdcx_stablecoin.aleo';
export const CREDITS_PROGRAM = import.meta.env.VITE_CREDITS_PROGRAM || 'credits.aleo';
...
export class AleoService {
  ...
  async getLoanCounter(): Promise<number> {
    try {
      const res = await fetch(`${this.base()}/program/${PROGRAM_ID}/mapping/loan_counter/0u32`);
      if (res.status === 404) return 0;
      if (!res.ok) return 0;
      const text = await res.text();
      const n = parseInt(text.replace(/['"]/g, '').replace(/u32$/i, '').trim(), 10);
      return isNaN(n) ? 0 : n;
    } catch {
      return 0;
    }
  ...
  async getLoanPublic(loanId: number): Promise<LoanPublic | null> {
    try {
      ...
      const [activeRes, ownerRes, lenderRes, deadlineRes, collateralRes] = await Promise.all([
      ...
      const collateralText = collateralRes.ok ? await collateralRes.text() : 'false';

      const active   = activeText.replace(/['"]/g, '').trim() === 'true';
      const owner    = ownerText.replace(/['"]/g, '').trim();
      const lender   = lenderText.replace(/['"]/g, '').trim();
      const deadline = parseInt(deadlineText.replace(/['"]/g, '').replace(/u32$/i, '').trim(), 10);
      const collateralLocked = collateralText.replace(/['"]/g, '').trim() === 'true';

      if (!owner || !lender || isNaN(deadline)) return null;

      return { loan_id: loanId, active, owner, lender, deadline, collateral_locked: collateralLocked };
    } catch {
      return null;
    }
  }
  ...
  async getLoansByLender(lender: string, maxLoanId: number): Promise<LoanPublic[]> {
    const all = await this.getAllLoans(maxLoanId);
  ...
  private async getMappingBalance(program: string, suffix: RegExp): Promise<(address: string) => Promise<bigint>> {
    return async (address: string) => {
      try {
        const res = await fetch(`${this.base()}/program/${program}/mapping/account/${address}`);
        if (!res.ok) return 0n;
  ...
  async getUSDCxBalance(address: string): Promise<bigint> {
    try {
      ...
      const res = await fetch(`${this.base()}/program/${USDCX_PROGRAM}/mapping/balances/${address}`);
      if (!res.ok) return 0n;
  ...
  async getCreditsBalance(address: string): Promise<bigint> {
    try {
      const res = await fetch(`${this.base()}/program/${CREDITS_PROGRAM}/mapping/account/${address}`);
      if (!res.ok) return 0n;
  ...
  async getTVL(maxLoanId: number): Promise<{ usdcx: bigint; credits: bigint }> {
    ...
    const lockedCount = loans
      .filter(l => l.active && l.collateral_locked)
      .length;
